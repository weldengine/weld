...
/// language validation accepted an invalid program.
///
/// The cycle is reported on the LAST-declared member, whose requisites all
/// resolve; the others emit `E0506` on a forward reference, which is
/// pre-existing and outside this review's perimeter — measured on the two-member
/// corpus case, which emits BOTH codes and not one.
const src_p2f_cycle =
    \\@requires(C1)
...
test "P2-F: a cycle longer than the old 64-name frontier is refused" {
    const gpa = std.testing.allocator;
    var codes: std.ArrayListUnmanaged([]const u8) = .empty;
    defer freeCodes(gpa, &codes);
    try diagCodes(gpa, src_p2f_cycle, &codes);
