//! AST stable interface — Level 1 guard.
//!
//! The COMPILATION of this file is the invariant (`etch-parser.md` §10.3.1 and
//! the contract block in `src/etch/root.zig`). It reaches thirty distinct
//! Level-1 public entry points of the frozen AST surface through the
//! `weld_etch` module boundary alone — never an internal path — so a rewrite
//! of the parser that removes or renames any of them breaks this file at
//! compile time rather than at a call site somewhere downstream.
//!
//! The delivered AST is a tabular SoA and not §10.3.1's idealized tagged-union
//! prose: four per-category kind enums, a `NodeId` handle, and parallel
//! kind/data/span accessors. This pins the real surface.

const std = @import("std");
const weld_etch = @import("weld_etch");

// A reference program yielding at least one node in every category (item, stmt,
...
test "AST Level-1 frozen surface: ≥20 distinct public entry points compile + resolve" {
    ...
    // Entry points 1-8: the eight frozen discrimination enums. Naming a variant
    // pins the enum's existence AND the variant's survival.
    ...
    // 9-10: the `NodeId` handle — packed u32, plus none/isNone/raw.
    ...
    // 11: `StringId`, the opaque intern handle (= u32).
    const sid_zero: weld_etch.StringId = 0; // (11)
    std.mem.doNotOptimizeAway(sid_zero);

    // 12: `SourceSpan` and its frozen fields.
    ...
    // 13: `parseSource` → `ParseResult { ast, diagnostics }`.
    var result = try weld_etch.parseSource(gpa, reference_src); // (13)
    defer result.deinit(gpa);
    try std.testing.expectEqual(@as(usize, 0), result.diagnostics.len);
    const arena = &result.ast;

    // 14: `Ast.isEmpty` and the SoA column lengths, which are public fields.
    ...
    // 15-17: the item accessor triplet (kind/data/span).
    ...
    // 18-20: the expr accessor triplet.
    ...
    // 21-23: the stmt accessor triplet.
    ...
    // 24-26: the type-node accessor triplet.
    ...
    // 27-28: the string-intern pool — find + slice round-trip.
    ...
    // 29-30: the trivia accessors, frozen for the language server.
