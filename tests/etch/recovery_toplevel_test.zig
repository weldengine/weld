...
//! sane constructs still land in the AST. This is top-level resync ALONE — the
//! full panic-mode cascade with virtual tokens and fine sync points is
//! `etch-parser.md` §11 and §23, and is not implemented.
...
test "top-level resync surfaces one diagnostic per broken construct and keeps sane constructs" {
    ...
    // Three constructs, the middle one broken — a field default with no value
    // expression (`= }`). Without the sync point the whole file aborts at the
    // first error; with it the parser records a diagnostic, resyncs at the next
    // `component` keyword, and still parses the two sane constructs around it.
...
test "recovery resyncs at a top-level `fn` after a broken construct (lockstep)" {
    ...
    // production, the function is not skipped and lands in the AST. Without that
    // lockstep the recovery loop runs past `fn double` to EOF and silently drops
    // a valid construct.
    var result = try etch.parseSource(gpa,
        \\component Broken { x: int = }
...
test "recovery resyncs at a top-level `async fn` after a broken construct (lockstep)" {
    const gpa = std.testing.allocator;
    // The `async` starter joined the stop-set in lockstep too, an `async fn`
    // being a top-level construct. A broken construct before one must not
    // swallow it.
    var result = try etch.parseSource(gpa,
        \\component Broken { x: int = }
...
test "recovery keeps a rule exercising the whole body-construct set after a broken construct" {
    const gpa = std.testing.allocator;
    // A broken component precedes a rule whose body exercises arrays and
    // indexing, closures and calls, for-in, loop/break and try/catch/throw. The
    // parser resyncs at `rule` and the whole body parses cleanly: none of those
    // body constructs adds a top-level keyword, so `recoverToTopLevel`'s stop-set
    // is unchanged and must stay sufficient.
    var result = try etch.parseSource(gpa,
        \\component Broken { x: int = }
