...
///
/// **THIS IS THE OTHER SIDE OF THE CONSERVATION.** `uncollectedOn` gives the
/// declared gap, so
...
pub fn expectedCollectedOn(os: std.Target.Os.Tag) usize {
    ...
    // reported `2244/2263 tests passed (19 skipped)` on macOS when these values were
    // last set, and the closure arrives at 2263 independently from the table above.
