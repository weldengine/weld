...
//! cache. Re-running logs a cache hit and skips the cook. This is the thin
//! offline surface; the user-facing `weld cook` CLI is Phase 1
//! (brief §Out-of-scope).
//!
//! Usage: `zig build cook-demo [-- <out_dir>]` (default out dir
//! `zig-out/cook-demo`).
...
pub fn main(init: std.process.Init) !void {
    ...
    for (fixtures) |fx| {
        ...
        var imp = try importOne(gpa, fx, src);
        ...
        const etch_name = try std.fmt.bufPrint(&name_buf, "{s}.{s}.asset.etch", .{ doc.name, fx.type_tag });
        ...
        if (cache.contains(io, &key)) {
            try stdout.print("  {s:<24} cache HIT (cook skipped)\n", .{fx.path});
        } else {
            const bin = try cookOne(gpa, fx, doc, imp.blob);
            defer gpa.free(bin);
            try cache.put(io, &key, bin);
            try writeFile(io, cooked_dir, bin_name, bin);
            try stdout.print("  {s:<24} cooked MISS → {s} ({d} bytes)\n", .{ fx.path, bin_name, bin.len });
        }
    }
    try stdout.flush();
}

fn importOne(gpa: std.mem.Allocator, fx: Fixture, src: []const u8) !assets.importers.Import {
    return switch (fx.kind) {
        .texture => try assets.importers.png.import(gpa, fx.path, src),
        .mesh => try assets.importers.gltf.import(gpa, fx.path, src),
        .audio => try assets.importers.wav.import(gpa, fx.path, src),
    };
}
