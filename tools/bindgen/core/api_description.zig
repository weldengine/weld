...
//!
//! **STRUCTURAL SKELETON — nothing in the tree goes through it.** The two
//! effective adapters, `vk_xml` and `wayland_xml`, write the idiomatic Zig
//! directly and never build an `ApiDescription`. The format is laid down and
//! frozen for the seven authorised keepers (Opus, Assimp, KTX/Basis,
//! libdatachannel, ACL compressor, HarfBuzz, ONNX), which have no retroactive
//! "empty diff" criterion to preserve.
//!
//! It is meant to become the **canonical input** of `core/emitter.zig` for those
//! adapters and for manual bindings (`bindings/manual/*.api.zig`). The
//! `bindings/generated/*.api.zig` the live adapters produce carry a minimal
//! `ApiDescription` filling in `name` / `version` / `source` — enough to tell a
//! manual description from a generated one and keep the pipeline auditable. The
//! full contract (types, functions, ownership, loading strategies) is exercised
//! by nothing today.
...
pub const Source = union(enum) {
    ...
    output_only,
    /// Manually written description (a keeper, via
...
/// Loading strategy of a lib. Four variants (cf.
/// `engine-c-bindings.md` §4.6); only `dlopen_loader_pattern`
/// is exercised, by the Vulkan and Wayland adapters.
...
/// Minimalist function declaration — skeleton for the
/// keepers.
