...
| Last released tag | `v0.10.9-extension-hooks` |
| Active branch | `main` |
| Next planned milestone | M1.1.0 — Forge 3D foundations (zolt): types + physics math + `BodyManager` (SoA aligned ECS). M1.0 (Etch ↔ ECS interpreter, M1.0.0–M1.0.9) is complete. `override` stays the last reserved `non_s3_keywords` member (waits for a Tier-1 overridable module). |
