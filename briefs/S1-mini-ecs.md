...
1. `engine-ecs-internals.md` § "Architecture" (ECS overview), `ARCH-005`, `ARCH-017` (in-tree extraction criterion), `engine-phase-minus-1-archive.md` S1 (canonical milestone definition).
...
- 2026-05-09 12:31 — Post-review fixes from Claude.ai PR review: documented why `Scheduler.init` keeps `gpa` unused in S1 (Phase 0.1 will allocate); replaced silent stale-fn fallback in `workerMain` with hard assert; replaced `assert(false)` on unknown despawn with `@panic`; clarified zero-alloc closing note on test scope. Status remains CLOSED.
...
- **What worked:** the comptime + work-stealing hypothesis is validated empirically on the M4 Pro reference (median **54.5 µs** for 100 000 entities, 18× under the 1.0 ms gate and 9× under the 0.5 ms secondary target). Load imbalance **3.46 %** — well below the 15 % gate. Zero allocations in the steady-state simulation loop after init: verified by `tests/ecs/no_alloc_in_simulation_test.zig` over 1000 iterations on the `query.forEachChunk` path. The `scheduler.dispatch` path is zero-alloc by construction (preallocated `chunks[]` array, atomic stores only) but is not covered by a dedicated test in S1 — added to Phase 0.1 testing scope. Chunk layout (16 KiB exact, 16-aligned per-component arrays, capacity 185 for `(Transform, Velocity)`) computes deterministically at comptime and survives the benchmark unchanged.
