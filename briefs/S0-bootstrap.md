...
- `<sha-du-commit-fix>` — `.vscode/settings.json` further revised: added `files.exclude` and `files.watcherExclude` blocks for `.zig-cache` / `zig-out` / `zig-pkg` (hides build artefacts from the VSCode tree and prevents the OS file watcher from receiving thousands of events per rebuild), and added `zig.buildOnSaveProvider: "zls"` so zls owns the build-on-save (coherent with `enable_build_on_save: true` in `.zls.json`, avoids double invocation between the extension and zls). The `zig.*` key order was tightened. The brief's IDE-config minimum was already exceeded by the previous commit; this is a follow-up refinement. Decided in Claude.ai post-review (revision of the original Fix 6 prescription).
