...
//!
//! The day-1-frozen surfaces every later stage builds on:
...
//!
//! Above them sit `codecs/` (DEFLATE, PNG, glTF), `importers/`, `cookers/`,
//! `cache/` and the async `loader/`. The public surface here stays the
//! frozen contract; implementation behind it evolves.
//!
//! Dependencies: `core` (the loader consumes the Tier 0 job system) and
//! `foundation` (the SIMD kernels). No `weld_etch` dependency.
...
/// Low-level codecs: DEFLATE/zlib, PNG, static glTF.
...
/// Async runtime loader + lifecycle.
