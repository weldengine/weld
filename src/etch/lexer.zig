...
test "lexer flags unknown Etch keyword from full grammar as error_unknown_keyword" {
    const gpa = std.testing.allocator;
    // `fn` graduated with the M0.8 E2 call mechanism, `enum` with E2 block 3
    // tranche B; `trait` (until tranche C) and `behavior` stay out of scope and
    // lex as error_unknown_keyword.
    var lex = Lexer.init("fn trait behavior");
    defer lex.deinit(gpa);
