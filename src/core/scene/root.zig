...
/// `.scene.bin` zero-copy accessor (read half; reused verbatim by the loader).
pub const accessor = @import("accessor.zig");
/// `.scene.bin` structural validator: a standalone pre-flight
...
/// `.scene.bin` runtime loader: reads a cooked scene back into a live
