...
/// Each job carries its own `trampoline` + `ctx_ptr` so
/// a single dispatch can run heterogeneous bodies — required by the
/// multi-job concurrent intra-phase scheduler which interleaves
...
/// the C0.1 bench worst case: 1 000 000 entities across 4
...
/// counts (at 4 workers a run handles ~640 chunks per
...
///
/// Exposed so the scheduler can size the dynamic
...
/// condvar — the sleep/wake replacement of a busy-yield.
pub const WorkerStats = struct {
    ...
    /// park↔wake churn — the state the parking test relies on) that is a worker
    /// parked right now. The deterministic parking test observes
    ...
    /// than busy-yielded). Used by the "idle workers sleep"
