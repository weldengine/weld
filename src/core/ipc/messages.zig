...
//! and the runtime.
//!
//! `WELD_IPC_PROTOCOL_VERSION` is 3, which covers the whole catalogue and the
//! attach semantics.
//!
//! The message structs are deliberately minimal: the runtime stub increments a
//! counter on `SpawnEntity` rather than wiring the real ECS, and
//! `ModifyComponent` is exercised only as a non-trivial payload shape. These
//! `extern struct` layouts survive into the real semantics unchanged.
//!
...
pub const MsgType = enum(u16) {
    ...
    /// Declared with **no wired handler** — wiring belongs to
    /// the scene serialization pipeline.
    save_scene = 20,
...
/// §5.1. `GPU_SHARED_FB` is locked at bit 0 and the runtime
/// stub publishes the capability bitfield at zero — stabilising
/// the `schema_hash` of `ProtocolHello` against a later introduction of
...
pub const ProtocolHello = extern struct {
    ...
    build_hash: [16]u8,
    /// Capability bitfield (cf. `Capability`). The stub publishes 0.
    capabilities: u32,
};
...
/// Editor → Runtime. Transactional. In the stub the runtime increments
/// a counter and replies with `EntityCreated`. The `archetype_hint`
/// field is informational only — kept so the struct is a non-zero-
/// sized extern struct and so a real archetype path can be wired
...
/// synthetic counter in the stub; it widens to a generational
...
/// The stub runtime echoes back via `ModifyAck` with `success = 1`.
...
/// 1 in stub mode (the runtime never rejects there).
...
/// the only unidirectional event (no ack expected).
...
/// `"viewport_framebuffer"` (20 bytes) is the longest name the runtime hands
...
/// Maximum shm regions carried by one `ShmRegionsHandoff`. The runtime hands
...
pub const ShmRegionsHandoff = extern struct {
    ...
    /// ancillary data). `1` today (`viewport_framebuffer` only).
    region_count: u32,
    _pad0: u32 = 0,
...
/// Conduit `scene.save`). Declared with **no wired handler**
...
/// anchors the editor `CommandLog.last_clean_line` (§7).
...
/// Comptime schema hash for a message type. Delegates to the Tier 0 RTTI subsystem
/// (`rtti.computeSchemaHash`), which replaced a hand-rolled hash. Call sites are
/// unchanged.
///
...
/// reference the base messages (`ProtocolHello`, `SpawnEntity`,
...
test "MsgType.isKnown rejects out-of-range values" {
    ...
    try std.testing.expect(MsgType.isKnown(14)); // shm_regions_handoff
    try std.testing.expect(MsgType.isKnown(23)); // runtime_error
