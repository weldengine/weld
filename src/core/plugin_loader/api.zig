...
//!
//! `WeldAPI` table + 7 sub-APIs with stub
//! implementations.
//!
//! All signatures are **frozen final** in the sense of the
//! C0.5 partial freeze. No callback actually
...
// `event_register`. Mirror of the RTTI `FieldDesc`.
// =============================================================

/// Discriminant tag of a `WeldFieldDesc`. Mirrors `rtti.FieldKind`
/// at the C ABI boundary — extended with `WELD_FIELD_*` variants
...
/// `rtti.FieldDesc` with extra editor hints (`range_min/max`,
/// `tooltip`, `group`).
...
/// `rtti.Lifecycle` — drives the serialization / replication
...
/// / false — the real wiring is unimplemented.
pub const WeldEcsAPI = extern struct {
    ...
    // WHEN this is bound, it must answer over the UNION of both
    // storage backends: a component is stored either in its
...
/// Events sub-API table (cf. `engine-c-api.md §7`). Stubbed —
/// the wiring is unimplemented.
...
/// Stubbed — the registry itself is unimplemented.
...
/// `frame`). Stubbed — every sub-API callback returns
...
// functions. The `Loader` passes `&stub_api` to plugins; the real
// runtime wiring is unimplemented.
// =============================================================
...
/// Stub API table used by `Loader.loadPlugin`.
