...
test "a view reads and writes exactly what its declaration grants" {
    ...
    const spec = [_]Access{
        Access.reads(world_mod.Velocity),
        Access.writes(world_mod.Transform),
    };
    const v = View(&spec).fromErased(@ptrCast(&world));

    // The POSITIVE witness. Without it every counter-proof below is vacuous:
    // a view that refuses everything would pass all three of them.
    v.getMut(world_mod.Transform, eid).?.pos[0] = 4;
    try testing.expectEqual(@as(f32, 4), v.get(world_mod.Transform, eid).?.pos[0]);
    try testing.expectEqual(@as(f32, 0), v.get(world_mod.Velocity, eid).?.linear[0]);

    try testing.expect(v.isLive(eid));
    try testing.expectEqual(@as(usize, 1), v.entityCount());
}
...
test "a view's getMut stamps the changed tick its read side then observes" {
    ...
    const spec = [_]Access{Access.writes(world_mod.Transform)};
    const v = View(&spec).fromErased(@ptrCast(&world));

    world.beginFrame();
    const before = v.changedTick(world_mod.Transform, eid).?;
    world.beginFrame();
    v.getMut(world_mod.Transform, eid).?.pos[1] = 1;
    const after = v.changedTick(world_mod.Transform, eid).?;
...
test "a view reaches a resource by its declared type and nothing else" {
    const gpa = testing.allocator;
    var world = World.init();
    defer world.deinit(gpa);
    ...
    try world.addResource(gpa, rid, std.mem.asBytes(&R1{ .v = 7 }));

    const spec = [_]Access{Access.writesResource(R1)};
    const v = View(&spec).fromErased(@ptrCast(&world));

    try testing.expectEqual(@as(u8, 7), v.resourceBytes(R1).?[0]);
    v.resourceBytesMut(R1).?[0] = 9;
    try testing.expectEqual(@as(u8, 9), v.resourceBytes(R1).?[0]);

    // An undeclared resource type is refused at compile time, so what stays
    // observable at run time is the absence of a resource that was never
    // published — a different question, and one the view still answers.
    const spec2 = [_]Access{Access.readsResource(R2)};
    const v2 = View(&spec2).fromErased(@ptrCast(&world));
    try testing.expect(v2.resourceBytes(R2) == null);
}
